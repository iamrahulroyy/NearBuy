...
class IDB:
    ...
    async def add_item(request: Request, data: ItemCreate, db_pool: Session):
        try:
            ...
            existing_item = await DB.get_attr_all(dbClassNam=ItemTableEnum.ITEM, db_pool=db_pool, filters={"itemName": data.itemName}, all=False)
            if existing_item:
                return send_json_response(message="Item already exists", status=status.HTTP_403_FORBIDDEN, body={})
    ...
    async def get_all_items(request: Request, db_pool: Session):
        try:
            ...
            serialized_items = [{k: v for k, v in item.items() if k != 'id'} for item in jsonable_encoder(items)]
    ...
    async def update_item(request: Request, data: ItemUpdate, db_pool: Session):
        try:
            if not data.itemName:
                return send_json_response(message="Item name is required for update", status=status.HTTP_403_FORBIDDEN, body={})
            ...
            identifier = {"itemName": data.itemName}
            ...
            updated_item = await DB.get_attr_all(dbClassNam=ItemTableEnum.ITEM, db_pool=db_pool, filters={"itemName": data.itemName}, all=False)
